...
TRIPLE_EXTRACTION_PROMPT = """你是一个跨 session 知识图谱构建助手。请从以下"任务摘要"中抽取 (实体, 关系, 实体) 三元组。
...
ENTITY_MERGE_PROMPT = """判断以下两个实体在给定的上下文中是否应该合并为同一个概念。
...
BATCH_ENTITY_MERGE_PROMPT = """你是一个知识图谱实体对齐助手。请判断以下实体对是否应该合并为同一个概念。
...
class KGBuilder:
    ...
    def _batch_llm_confirm_merges(
        ...
        prompt = BATCH_ENTITY_MERGE_PROMPT.format(pairs_text="\n".join(lines))
