...
【task_summary 写作要求】
每个 task 的 summary 必须是语义自洽的知识块（彻底去除“用户”、“助手”等对话痕迹），且需要有逻辑层次，而非机械罗列：
- 背景与目标：一句话说明任务的背景和要解决的问题（为什么做）。
- 核心产出与关键决策：说明做了什么、怎么做的，必须保留关键技术实体和参数，以便于后续构建知识图谱和精准检索。
- 结论与意义：点明最终结果、业务建议或影响（结果如何）。
好的例子: “为深入理解 LLM 训练硬件选型，系统对比了 V100/A100/A800/H100/H200/H800 六款 GPU 在算力、显存带宽和互联能力上的差异。结论：基于 Hopper 架构的 H100/H200 适合大规模训练；H800/A800 因 NVLink 带宽阉割至 400GB/s 不适合多机多卡场景；A800 是当前国内性价比合规选择。”
...
{{
  ...
  "tasks": [
    {{
      ...
      "task_summary": "有逻辑层次的总结(50-200 字): 背景目标 → 关键决策/产出 → 结果意义",
      "chunk_ids": ["c1", "c2", "c3"]
    }}
  ]
}}
